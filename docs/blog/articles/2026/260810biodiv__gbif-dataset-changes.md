...
- **當初是為了某個短期計畫而發布**：計畫有期程，上架的內容通常侷限在那幾個年度、那幾個類群；但原始資料後端資料庫（非IPT）還是持續累積資料，久了計畫版的資料集就跟現況差很多。
- **發布單位變動**：早期不少資料雖然是由TaiBIF代為上架，發布單位是掛數位典藏計畫（TELDAP）或計畫所屬單位（林務局），不是原始資料的產生者。後來各單位自己註冊成為 GBIF 的發布單位，資料就搬到自己名下重新發布。
- **處理資料的人員異動**：原本資料的匯出流程沒有交接，很難接續，所以乾脆重新整理發布。
- **處理資料的流程異動**：可能因為管理系統更新、對資料填入資料標準有不同的處理方式、或是發布工具的更新，舊資料集難以更新，就會另外發布一個資料集。就資料發布工具來說，早期是用TAPIR（2009以前），而不是現在通用的[IPT](https://www.gbif.org/zh-tw/ipt)。
